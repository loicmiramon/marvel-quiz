...
const QuizOver = React.forwardRef((props, ref) => {
  ...
  let api = process.env.REACT_APP_MARVEL_API_KEY
  let hash = process.env.REACT_APP_HASH_API
  ...
  const showModal = id => {
    ...
    if(localStorage.getItem(id)) {
      ...
      axios.get(`https://gateway.marvel.com/v1/public/characters/${id}?ts=1&apikey=${api}&hash=${hash}`).then(response => {
        ...
        <div className='comicDetails'>
          ...
          <h3>Plus D'infos</h3>
          {
            ...
            characterInfos.data.results[0].urls.map((url, index) => {
              return <a key={index} href={url.url} target="blank" rel="noopener noreferrer">{capitalizeFisrtLetter(url.type)}</a>
            })
          }
        </div>
      </div>
      <div className="modalFooter">
        <button className="modalBtn" onClick={hideModal}>Fermé</button>
      </div>
    </>
  ) : (
    <>
